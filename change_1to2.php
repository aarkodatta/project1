...
$sql="SELECT NAME,ROLL,SUBCODE FROM ALL_STUDENTS_UG WHERE SUBCODE LIKE '____1%';";
...
if ($result->num_rows>0)
{
    while ($row=$result->fetch_assoc())
    {
        ...
        $ar[$c][1]=$row["ROLL"];
        $s=$row["SUBCODE"];
        $s1=substr($s,0,4)."2".substr($s,5,2);
        $ar[$c][2]=$s1;
        $c=$c+1;
    }
}
...
$conn->query($sql2);
$sql2="DELETE FROM ALL_STUDENTS_UG WHERE SUBCODE LIKE '____1%';";
$conn->query($sql2);
