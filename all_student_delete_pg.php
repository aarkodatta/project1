...
if (isset($_POST['submit']))
{
    ...
    $sql4="DELETE FROM ALL_STUDENTS_PG WHERE ROLL='';";
    $conn->query($sql4);
    $sql="SELECT NAME FROM ALL_STUDENTS_PG WHERE ROLL='$roll' AND SUBCODE LIKE '$sub%';";
    $rslt=$conn->query($sql);
    if ($rslt->num_rows > 0)
    {
        $sql3="DELETE FROM ALL_STUDENTS_PG WHERE ROLL='$roll' AND SUBCODE LIKE '$sub%';";
        $conn->query($sql3);
